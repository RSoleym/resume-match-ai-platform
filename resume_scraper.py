...
import os, re, json, hashlib, time, math, concurrent.futures
from datetime import date

from matcher_taxonomy import infer_resume_profile
...
INPUT_DIR = os.environ.get("ROLEMATCHER_RESUMES_DIR") or os.path.join(BASE_DIR, "resumes")
OUTPUT_JSON = os.environ.get("ROLEMATCHER_SCANNED_RESUMES_JSON") or os.path.join(BASE_DIR, "scanned_resumes.json")
MANIFEST_JSON = os.environ.get("ROLEMATCHER_RESUMES_MANIFEST_JSON") or os.path.join(BASE_DIR, "resumes_manifest.json")

TESSERACT_CONFIG = r"--oem 3 --psm 6"
OCR_PAGE_WORKERS = min(max(1, int(os.environ.get("ROLEMATCHER_OCR_PAGE_WORKERS", "3"))), max(1, (os.cpu_count() or 2)))
...
def _ocr_pdf_page(args):
    ...
    try:
        ...
        for img in images:
            try:
                gray = img.convert("L")
                page_texts.append(pytesseract.image_to_string(gray, config=TESSERACT_CONFIG))
            finally:
                try:
                    img.close()
                except Exception:
...
def ocr_pdf(path: str) -> str:
    ...
    workers = min(OCR_PAGE_WORKERS, len(page_numbers))
    if workers > 1:
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
                ordered = sorted(ex.map(_ocr_pdf_page, [(path, n) for n in page_numbers]), key=lambda x: x[0])
...
def scan_file(path: str) -> dict:
    ...
    inferred = infer_resume_profile(raw, parsed)
    ...
    return {
        ...
        "candidate_country": inferred.get("candidate_country", ""),
        "candidate_experience_years": inferred.get("candidate_experience_years"),
        "candidate_degree_level": inferred.get("candidate_degree_level", "none"),
        "candidate_degree_family": inferred.get("candidate_degree_family", "General"),
        "candidate_degree_fields": inferred.get("candidate_degree_fields", []),
        "candidate_category": inferred.get("candidate_category", "General"),
        "candidate_category_family": inferred.get("candidate_category_family", "General"),
        "candidate_category_confidence": inferred.get("candidate_category_confidence", 0.0),
        "candidate_function": inferred.get("candidate_function", ""),
        "candidate_function_confidence": inferred.get("candidate_function_confidence", 0.0),
        "candidate_function_runner_up": inferred.get("candidate_function_runner_up", ""),
        "candidate_function_scores": inferred.get("candidate_function_scores", {}),
        "candidate_domain": inferred.get("candidate_domain", ""),
        "candidate_domain_confidence": inferred.get("candidate_domain_confidence", 0.0),
        "candidate_domain_runner_up": inferred.get("candidate_domain_runner_up", ""),
        "candidate_domain_scores": inferred.get("candidate_domain_scores", {}),
        "candidate_category_key": inferred.get("candidate_category_key", ""),
        "candidate_category_key_confidence": inferred.get("candidate_category_key_confidence", 0.0),

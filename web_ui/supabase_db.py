...
class SupabaseDB:
    ...
    def _storage_url(self, path: str = "") -> str:
        base = self.config.url.rstrip("/") + "/storage/v1"
        if not path:
            return base
        return f"{base}/{path.lstrip('/')}"

    def upload_bytes(self, bucket: str, object_path: str, data: bytes, *, content_type: str = "application/octet-stream", upsert: bool = True) -> None:
        headers = {
        ...
        encoded = quote(object_path.strip("/"), safe="/")
        resp = requests.post(
            self._storage_url(f"object/{quote(bucket)}/{encoded}"),
            headers=headers,
            data=data,
            timeout=60,
        )
    ...
    def delete_storage_objects(self, bucket: str, object_paths: List[str]) -> Any:
        headers = dict(self.headers)
        payload = {"prefixes": [p.strip("/") for p in object_paths if str(p).strip()]}
        resp = requests.delete(
            self._storage_url(f"object/{quote(bucket)}"),
            headers=headers,
    ...
    def move_storage_object(self, bucket: str, source_path: str, destination_path: str) -> Any:
        headers = dict(self.headers)
        payload = {
            "bucketId": bucket,
            "sourceKey": source_path.strip("/"),
            "destinationKey": destination_path.strip("/"),
        }
        resp = requests.post(
